...
simulation <- function(Param.df, type, exp_type, save=F, savepath="",
  ...
  expschema = read.csv(paste0("suppfile_share/", exp_type, "_schemainfo2.csv"))
  ...
  for (Subject in 1:subjectnum) {
    ...
    }#这是一个线性映射。如果 scale.confi.init 为 False，它将原始数据中的熟悉度（familiarity）映射到 [0.2, 1.0] 区间。
    ...
    while (clock < 4500){# new clock < 4500 i.e. total time is (85-10)*60 seconds
      ...
      # artificial clock signal if we only simulate afterbreak 如果当前设置是模拟休息后，直接将时钟跳转到1500秒（休息结束）
      ...
      # 选择第一轮展示的schema
      #只处理了第一轮的初始化，它的核心目的是为了让模拟开局更“真实”——避免模型一开始就死盯着最高分的题目
      ...
      while (decisiontime <= 2){ ## i.e. 2 phase to choose item
        ...
        #### firstly, we calculate the DMs for each schema DM就是决策值
        Schema_EI$weightCon = w*Schema_EI$Scon + (1-w)*Schema_EI$Gcon #加权信心：信心=w×特定Schema信心+(1−w)×通用信心
  ...
  allresult$total_RT <- allresult$RT_1+allresult$RT_2+allresult$RT_3+allresult$RT_4

  #modify the result for saving: including accuracy, performance, RT
  ac_perf <- allresult %>%
    select(Subject, Round, Phase, total_RT, AC, performance, afterbreak, RT_1, RT_2, RT_3, RT_4)
  ...
  #   }
  #   filename <- param_comb
  #   write.csv(ac_perf, file.path(simupath, filename))
  #   # write_csv(ALL.df,file.path(simupath,'allresult.csv'))
  #   #write.csv(Param.df, file.path(simupath,'Paras.csv'))
    
  #   if(
  #     sim.mode == "before" | 
  #     save.confi == T){
  #     write.csv(confidence.df, file.path(simupath,'confidence.csv'))
  #     write.csv(gconfidence.df, file.path(simupath,'gconfidence.csv'))
  #   }
...
# helper: build the standard Param.df
build_base_param_df <- function(subjectnum) {
  data.frame(
...
# helper: apply a named parameter list to Param.df
apply_params <- function(Param.df, params) {
  for (name in names(params)) {
    if (!name %in% colnames(Param.df)) {
      stop(paste0("Unknown parameter: ", name))
    }
    Param.df[[name]] <- params[[name]]
  }
  Param.df
}
...
# default values used for file naming
default_param_values <- list(
  thres_item_final = 13.75,
  thres_item_inter = 6,
  thres_schema = 50,
  w = 0.30,
  theta_shift = 3,
...
# helper: build filename from parameter values, only include changed params
build_param_comb <- function(params, defaults = default_param_values) {
  changed <- params[sapply(names(params), function(name) {
    !identical(params[[name]], defaults[[name]])
  })]

  if (length(changed) == 0) {
    return("ac_perf_default.csv")
  }

  paste0(
    "ac_perf_",
    paste0(names(changed), "=", unlist(changed), collapse = "_"),
    ".csv"
  )
}

# batch run configuration: defining all parameter combinations for fine-tuning
param_sets <- list(
  # default: list(thres_item_final = 13.75, thres_schema = 50, w = 0.30),

  #thres_item_final
  # list(thres_item_final = 13.65, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 13.55, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 13.35, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 12.5, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 12, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 10, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 8, thres_schema = 50, w = 0.30),

  #thres_item_inter
  # list(thres_item_final = 13.75, thres_item_inter = 5.5, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 13.75, thres_item_inter = 5, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 13.75, thres_item_inter = 4, thres_schema = 50, w = 0.30),
  # list(thres_item_final = 13.75, thres_item_inter = 3, thres_schema = 50, w = 0.30),

  #thres_schema
  # list(thres_item_final = 13.75, thres_schema = 48, w = 0.30),
  # list(thres_item_final = 13.75, thres_schema = 46, w = 0.30),
  # list(thres_item_final = 13.75, thres_schema = 45, w = 0.30),
  # list(thres_item_final = 13.75, thres_schema = 40, w = 0.30),
  # list(thres_item_final = 13.75, thres_schema = 35, w = 0.30),
  # list(thres_item_final = 13.75, thres_schema = 30, w = 0.30)

  # #w
  list(thres_item_final = 13.75, thres_schema = 50, w = 0.4),
  # list(thres_item_final = 13.75, thres_schema = 50, w = 0.5),
  # list(thres_item_final = 13.75, thres_schema = 50, w = 0.7),
  ...
  #combining
  #list(thres_item_final = 13.65, thres_schema = 35, w = 0.5, Phi=25)
...
# output directory
base_path <- "./res/tmp_modified_new"
if (!dir.exists(base_path)) dir.create(base_path, recursive = TRUE)


for (i in seq_along(param_sets)) {
  params <- param_sets[[i]]
  message("Running batch ", i, " / ", length(param_sets), ": ", build_param_comb(params))

  Param.df <- build_base_param_df(subjectnum)
  Param.df <- apply_params(Param.df, params)
  ...
  ac_perf <- res$ac_perf
  param_comb <- build_param_comb(params)
  write.csv(ac_perf, file.path(base_path, param_comb), row.names = FALSE)
}
